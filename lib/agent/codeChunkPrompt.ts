...
const TOKEN_EXTRACT: string[] = [
  ...
  "Be EXHAUSTIVE — this is the whole point. Do NOT skip a token just because it feels basic or obvious. If it appears in the code, include it. In particular, do NOT omit: the assignment `=`; the ternary conditional (list it as the single token `? :` with a label like 'ternary / conditional'); comparison operators (`===`, `!==`, `==`, `<`, `>`, `<=`, `>=`); arithmetic (`+`, `-`, `*`, `/`, `%`); logical (`&&`, `||`, `!`, `??`, `?.`); other operators (`=>`, `...`); keywords (`const`, `let`, `var`, `function`, `return`, `import`, `export`, `export default`, `new`, `typeof`, `instanceof`, `in`, `of`, `async`, `await`, `void`, `interface`, `type`); built-in types/utilities (`boolean`, `string`, `number`, `Record`, `Array`, generics `<T>`); hooks/methods (`useState`, `useMemo`, `useEffect`, `useRef`, `useCallback`, `map`, `filter`, `reduce`); runtime/DOM APIs (`new Date`, `setTimeout`, `addEventListener`, `matchMedia`, `preventDefault`); JSX basics (`className`, `<div>`, `<button>`, standard handlers like `onClick`); and event parameters like `e` / `event`.",
