...
const TOKEN_EXTRACT: string[] = [
  "Also fill a `tokens` array with the UNIVERSAL, reusable tokens that ACTUALLY APPEAR in this code — things that mean the same in any codebase and are worth a beginner learning once: operators, keywords, built-in types, built-in/runtime APIs, hooks and common array/object methods, and DOM/JSX basics.",
  "Be EXHAUSTIVE — this is the whole point. Read the code token by token and include EVERY universal token you pass, even single characters and the most basic ones. Do NOT skip a token because it feels obvious. The catalog below lists common universal tokens by family; include ANY of them that appear, AND any other universal token even if it is not in this catalog (the catalog is a reminder, not a limit).",
  "Assignment & related operators: `=`, `+=`, `-=`, `*=`, `/=`, `%=`, `**=`, `??=`, `||=`, `&&=`.",
  "Comparison operators: `===`, `!==`, `==`, `!=`, `<`, `>`, `<=`, `>=`.",
  "Arithmetic operators (INCLUDING subtraction — `-` used to subtract numbers counts even though `-` also appears inside strings/class names): `+`, `-`, `*`, `/`, `%`, `**`, `++`, `--`.",
  "Logical / conditional operators: `&&`, `||`, `!`, `??`, `?.`, and the ternary conditional — list the ternary as the SINGLE token `? :` (label it 'ternary / conditional').",
  "Bitwise operators (when present): `&`, `|`, `^`, `~`, `<<`, `>>`, `>>>`.",
  "Other operators/punctuation: `=>` (arrow), `...` (spread/rest), `.` (member access), `?` (optional property/param marker in types), `:` (type annotation / object key), and template-literal interpolation `${ }`.",
  "Control-flow keywords: `if`, `else`, `for`, `while`, `do`, `switch`, `case`, `default`, `break`, `continue`, `return`, `try`, `catch`, `finally`, `throw`.",
  "Declaration / module / class keywords: `const`, `let`, `var`, `function`, `import`, `export`, `export default`, `from`, `as`, `new`, `delete`, `typeof`, `instanceof`, `in`, `of`, `this`, `super`, `class`, `extends`, `implements`, `static`, `get`, `set`, `public`, `private`, `protected`, `readonly`, `abstract`, `enum`, `namespace`, `declare`, `async`, `await`, `yield`, `void`, `interface`, `type`.",
  "Built-in types & TS utilities: `string`, `number`, `boolean`, `any`, `unknown`, `never`, `void`, `null`, `undefined`, `object`, `symbol`, `bigint`, literals `true`/`false`, `Array`, `Record`, `Map`, `Set`, `Promise`, `Partial`, `Pick`, `Omit`, `Readonly`, `ReturnType`, and generic type parameters like `<T>`.",
  "Built-in globals & runtime/DOM APIs: `console.log`, `JSON.parse`, `JSON.stringify`, `Object.keys`/`values`/`entries`/`assign`, `Array.isArray`, `Math.*`, `Number`, `String`, `Boolean`, `parseInt`, `parseFloat`, `new Date`, `setTimeout`, `setInterval`, `clearTimeout`, `Promise.all`, `fetch`, `localStorage`, `sessionStorage`, `addEventListener`, `removeEventListener`, `matchMedia`, `querySelector`, `preventDefault`, `stopPropagation`.",
  "Common array/object/string methods: `map`, `filter`, `reduce`, `forEach`, `find`, `findIndex`, `some`, `every`, `includes`, `push`, `pop`, `shift`, `unshift`, `slice`, `splice`, `concat`, `join`, `split`, `sort`, `reverse`, `flat`, `flatMap`, `keys`, `values`, `entries`, `replace`, `trim`, `toLowerCase`, `toUpperCase`, `startsWith`, `endsWith`, `padStart`, `repeat`.",
  "React & JSX: hooks (`useState`, `useEffect`, `useMemo`, `useCallback`, `useRef`, `useContext`, `useReducer`), JSX element tags (`<div>`, `<button>`, `<span>`, `<input>`, `<img>`, `<a>`, `<ul>`, `<li>`, `<Fragment>`/`<>`), and standard JSX attributes/handlers (`className`, `style`, `key`, `ref`, `type`, `value`, `disabled`, `onClick`, `onChange`, `onSubmit`, `children`).",
