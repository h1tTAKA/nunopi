...
const TOKEN_EXTRACT: string[] = [
  ...
  "Be EXHAUSTIVE — this is the whole point. Read the code token by token and include EVERY universal token you pass, even single characters and the most basic ones. Do NOT skip a token just because it feels obvious. In particular, do NOT omit: the assignment `=`; the ternary conditional (list it as the single token `? :` with a label like 'ternary / conditional'); comparison operators (`===`, `!==`, `==`, `<`, `>`, `<=`, `>=`); arithmetic INCLUDING subtraction (`+`, `-`, `*`, `/`, `%`) — `-` used to subtract numbers counts even though `-` also shows up inside strings/class names; logical (`&&`, `||`, `!`, `??`, `?.`); other operators (`=>`, `...`); control-flow keywords (`if`, `else`, `for`, `while`, `switch`, `case`, `break`, `continue`, `try`, `catch`, `finally`, `throw`); declaration/module keywords (`const`, `let`, `var`, `function`, `return`, `import`, `export`, `export default`, `new`, `typeof`, `instanceof`, `in`, `of`, `async`, `await`, `void`, `interface`, `type`); built-in types/utilities (`boolean`, `string`, `number`, `Record`, `Array`, generics `<T>`); hooks/methods (`useState`, `useMemo`, `useEffect`, `useRef`, `useCallback`, `map`, `filter`, `reduce`); runtime/DOM APIs (`new Date`, `setTimeout`, `addEventListener`, `matchMedia`, `preventDefault`); JSX element tags (`<div>`, `<button>`, `<span>`); standard JSX attributes/handlers (`className`, `style`, `key`, `type`, `disabled`, `onClick`, `onChange`); and event parameters and their type (`e` / `event`, `KeyboardEvent`, `MouseEvent`).",
