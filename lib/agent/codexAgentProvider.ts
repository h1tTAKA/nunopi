...
function buildCodexPrompt(request: AgentAnalyzeRequest): string {
  return [
    ...
    "Tokenize granularly: for EACH line, emit a token for EVERY meaningful token on that line — identifiers, keywords (const, return), operators (=, =>), and meaningful punctuation ([, ], ;, ,) — and reference ALL of them in that line's lineExplanations.tokenIds. Reuse one token id for a token text that recurs (collect its lines in tokens[].lines).",
