...
function buildCodexPrompt(request: AgentAnalyzeRequest): string {
  return [
    ...
    "Token granularity: include identifiers, keywords, operators, and meaningful punctuation, but emit ONE entry per DISTINCT token text — reuse its id across lines and collect the lines in tokens[].lines. NEVER repeat a token entry. lineExplanations.tokenIds reference the distinct tokens that appear on that line.",
    "Be concise to stay fast on long inputs (response time grows with output size): each line explanation is ONE short sentence; each token description is one short phrase; set example only when it genuinely helps (omit otherwise); summary is 2-4 sentences. Keep the essence, do not pad.",
