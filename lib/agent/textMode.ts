...
export function buildTextPrompt(request: AgentAnalyzeRequest): string {
  ...
  return [
    ...TEXT_HEADER,
    "",
    "Coverage & output order (results stream to the user as they are generated):",
    "- Output the JSON fields in THIS EXACT ORDER: title, then summary, then terms, then concepts.",
    "  (The reader sees each piece the moment it finishes, so order and per-item completeness matter.)",
