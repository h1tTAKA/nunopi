...
import type { SrsSource } from "@mustard/core";

// 부채꼴/더미 카드 — 가운데 플립 카드(앞면)와 같은 디자인: 흰 바탕 + 이중 프레임 + 중앙 심볼.
// 부모(카드 span, relative)를 꽉 채운다. 크기 무관하게 비율(%) 프레임.
// source 주면 그 분류 색(token=핑크/concept=에메랄드/term=파랑), 없으면 term(파랑) 기본.
export default function CardBack({ source = "term" }: { source?: SrsSource }) {
  const f = cardFrame(source);
  return (
    <span className="absolute inset-0 overflow-hidden rounded-[inherit] bg-white">
      <span className={`pointer-events-none absolute inset-[6%] rounded-[10%] border-2 ${f.outer}`} />
      <span className={`pointer-events-none absolute inset-[9%] rounded-[8%] border ${f.inner}`} />
