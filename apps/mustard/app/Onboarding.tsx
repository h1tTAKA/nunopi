...
// gradient 없음, 일관 rounded/보더/틴트. 다크 전용(라이트 모드 수정 전). 테마 선택은 제외.
        ...
        <div className="w-full max-w-[480px] rounded-2xl border border-zinc-800 bg-zinc-900/50 p-8 shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
          ...
          <div className="space-y-6">
                ...
                {LOCALES.map((l) => {
                  ...
                  return (
                    <button key={l.value} type="button" onClick={() => setLocale(l.value as Locale)}
                      className={`rounded-md px-4 py-1.5 text-sm transition ${on ? "bg-zinc-700 font-medium text-zinc-50" : "text-zinc-400 hover:text-zinc-200"}`}>
                      {l.label}
                    </button>
                  );
                })}
            ...
            <section className="space-y-2">
              ...
              <div className="rounded-xl border border-zinc-800 bg-zinc-800/25 px-4 py-3.5">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex min-w-0 items-center gap-2.5">
                    <IconSparkles size={18} stroke={1.75} className={nunopi ? "text-mustard-400" : "text-zinc-500"} aria-hidden />
                    <span className="text-sm font-medium text-zinc-100">nunopi</span>
                  </div>
                  <button type="button" role="switch" aria-checked={nunopi} aria-label={t("onboarding.nunopiLabel")}
                    onClick={() => setNunopi((v) => !v)}
                    className={`relative h-5 w-9 shrink-0 rounded-full transition-colors ${nunopi ? "bg-mustard-500" : "bg-zinc-700"}`}>
                  ...
                  {CLI.map(({ key, label, ph }) => (
                    <div key={key} className="flex items-center gap-3">
                      ...
                      <input type="text" value={paths[key] ?? ""} placeholder={ph}
                        ...
                        className="min-w-0 flex-1 rounded-md border border-zinc-800 bg-zinc-900 px-3 py-1.5 font-mono text-[13px] text-zinc-100 outline-none transition placeholder:text-zinc-600 focus:border-zinc-600" />
                    </div>
                  ))}
                </div>
              </section>
