...
interface NunopiDesktopApi {
  ...
  terminal: {
    ...
    // sessionTitle(#970)=세션 작업 제목(OSC 타이틀 요약) | ""(셸·미판정). 탭 라벨·호버 이름용.
    list(): Promise<{ id: string; cwd: string; process: string; pid: number; agent: string | null; sessionTitle: string }[]>;
